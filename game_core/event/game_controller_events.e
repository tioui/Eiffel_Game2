note
	description: "Manage Controller events"
...
feature {NONE} -- Initialisation
	...
	make
		...
		do
			...
			button_pressed_events_callback := agent (a_timestamp: NATURAL_32; a_controller_id:INTEGER_32; a_button_id:NATURAL_8)
				do
					button_pressed_events_dispatcher(a_timestamp, a_controller_id, a_button_id)
				end
			button_released_events_callback := agent (a_timestamp: NATURAL_32; a_controller_id:INTEGER_32; a_button_id:NATURAL_8)
				do
					button_released_events_dispatcher(a_timestamp, a_controller_id, a_button_id)
				end
...
feature -- Access
	...
	stop
		...
		do
			is_running := False
			events_controller.controller_axis_motion_actions.prune_all (axis_motion_events_callback)
			events_controller.controller_button_pressed_actions.prune_all (button_pressed_events_callback)
			events_controller.controller_button_released_actions.prune_all (button_released_events_callback)
		end
	...
	run
		...
		do
			...
			if attached axis_motion_actions_internal then
						events_controller.controller_axis_motion_actions.extend (axis_motion_events_callback)
					end
			if attached button_pressed_actions_internal then
				events_controller.controller_button_pressed_actions.extend (button_pressed_events_callback)
			end
			if attached button_released_actions_internal then
				events_controller.controller_button_released_actions.extend (button_released_events_callback)
			end
		end
	...
	axis_motion_actions: ACTION_SEQUENCE [TUPLE [timestamp: NATURAL_32; axis_id:NATURAL_8; value:INTEGER_16]]
		...
		require
			controller_Axis_Motion_Event_Enable: events_controller.is_controller_axis_motion_event_enable
		do
			if attached axis_motion_actions_internal as la_axis_motion_actions_internal then
				Result := la_axis_motion_actions_internal
			else
				create Result
				if is_running and not events_controller.controller_axis_motion_actions.has (axis_motion_events_callback) then
					events_controller.controller_axis_motion_actions.extend (axis_motion_events_callback)
				end
	...
	button_pressed_actions: ACTION_SEQUENCE [TUPLE [timestamp: NATURAL_32; button_id:NATURAL_8]]
		...
		require
			Controller_Button_Pressed_Event_Enable: events_controller.is_controller_button_pressed_event_enable
		do
			if attached button_pressed_actions_internal as la_button_pressed_actions_internal then
				...
				create Result
				if is_running and not events_controller.controller_button_pressed_actions.has (button_pressed_events_callback) then
					events_controller.controller_button_pressed_actions.extend (button_pressed_events_callback)
				end
	...
	button_released_actions: ACTION_SEQUENCE [TUPLE [timestamp: NATURAL_32; button_id:NATURAL_8]]
		...
		require
			controller_Button_Released_Event_Enable: events_controller.is_controller_button_released_event_enable
		do
			if attached button_released_actions_internal as la_button_released_actions_internal then
				...
				create Result
				if is_running and not events_controller.controller_button_released_actions.has (button_released_events_callback) then
					events_controller.controller_button_released_actions.extend (button_released_events_callback)
				end
	...
	removed_actions: ACTION_SEQUENCE [TUPLE [timestamp: NATURAL_32]]
		...
		require
			controller_Button_Pressed_Event_Enable: events_controller.is_controller_device_removed_event_enable
		do
...
feature {NONE} -- Implementation
	...
	button_pressed_events_callback: PROCEDURE [ANY, TUPLE [timestamp: NATURAL_32; controller_id: INTEGER_32; button_id:NATURAL_8]]
			-- Callback used to register `Current' in the `events_controller' for the
			-- `button_pressed_actions' {ACTION_SEQUENCE}

	button_pressed_events_dispatcher (a_timestamp: NATURAL_32; a_controller_id:INTEGER; a_button_id:NATURAL_8)
	...
	button_released_events_callback: PROCEDURE [ANY, TUPLE [timestamp: NATURAL_32; controller_id: INTEGER_32; button_id:NATURAL_8]]
			-- Callback used to register `Current' in the `events_controller' for the
			-- `button_released_actions' {ACTION_SEQUENCE}

	button_released_events_dispatcher (a_timestamp: NATURAL_32; a_controller_id:INTEGER_32; a_button_id:NATURAL_8)
	...
	removed_events_dispatcher (a_timestamp: NATURAL_32; a_controller_id:INTEGER)

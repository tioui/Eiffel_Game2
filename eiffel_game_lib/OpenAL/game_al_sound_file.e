...
feature {GAME_AL_SOURCE}
	...
	fill_buffer(buffer:POINTER;max_length:INTEGER)
		do
			last_buffer_size:={GAME_AL_EXTERNAL}.sf_read_short(snd_file_ptr,buffer,max_length//2).to_integer
		end
...
feature --Access
	get_channels:INTEGER
		...
		do
			Result:={GAME_AL_EXTERNAL}.get_sf_info_struct_channels(file_info)
		end

	get_frequency:INTEGER
			-- Get the frequency (sample rate) of the sound.
		do
			Result:={GAME_AL_EXTERNAL}.get_sf_info_struct_samplerate(file_info)
		end
	...
	is_seekable:BOOLEAN
		...
		do
			Result:=({GAME_AL_EXTERNAL}.get_sf_info_struct_seekable(file_info)/=0)
		end
	...
	restart
		...
		do
			if is_seekable then
				error:={GAME_AL_EXTERNAL}.SF_seek(snd_file_ptr,0,{GAME_AL_EXTERNAL}.SEEK_SET)
				check error/=-1 end
			else
				dispose
...
feature {NONE} -- Implementation - Methodes
	...
	open_from_file(filename:STRING)
		local
			filename_c:C_STRING
		do
			file_info:={GAME_AL_EXTERNAL}.c_sf_info_struct_allocate
			create filename_c.make(filename)
			snd_file_ptr:={GAME_AL_EXTERNAL}.SF_open(filename_c.item,{GAME_AL_EXTERNAL}.SFM_READ,file_info)
	...
	dispose
		local
			error:INTEGER
		do
			{GAME_AL_EXTERNAL}.c_sf_info_struct_free(file_info)
			error:={GAME_AL_EXTERNAL}.sf_close(snd_file_ptr)
			check error=0 end
		end

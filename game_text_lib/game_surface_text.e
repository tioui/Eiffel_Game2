...
feature {NONE} -- Initialization
	...
	make_solid(l_text:STRING_8;l_font:GAME_FONT;l_color:GAME_COLOR)
		...
		do
			...
			make_from_pointer ({GAME_TEXT_EXTERNAL}.TTF_RenderText_Solid(l_font.sdl_font_pointer,text_c.item,l_sdl_color.sdl_color_pointer))
			enable_alpha
		ensure
	...
	make_shaded(l_text:STRING_8;l_font:GAME_FONT;l_color,l_color_bg:GAME_COLOR)
		...
		do
			...
			make_from_pointer ({GAME_TEXT_EXTERNAL}.TTF_RenderText_Shaded(l_font.sdl_font_pointer,text_c.item,l_sdl_color.sdl_color_pointer,l_sdl_color_bg.sdl_color_pointer))
		ensure
			SDL_Surface_Text_Surface_Imp_Ok: surface_imp = Void or surface_imp.is_surface_ok
		end
	...
	make_blended(l_text:STRING_8;l_font:GAME_FONT;l_color:GAME_COLOR)
		...
		do
			...
			create text_c.make (l_text)
			create l_sdl_color.make_from_rgba_color (l_color)
			make_from_pointer ({GAME_TEXT_EXTERNAL}.TTF_RenderText_Blended(l_font.sdl_font_pointer,text_c.item,l_sdl_color.sdl_color_pointer))
			enable_alpha
		ensure
			SDL_Surface_Text_Surface_Imp_Ok: surface_imp = Void or surface_imp.is_surface_ok
		end

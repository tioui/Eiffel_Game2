...
feature {NONE} -- Initialization
	...
	run_standard(lib_ctrl:GAME_LIB_CONTROLLER)
		...
		do
			...
			reader.auto_resize:=false	-- When the movie will loop or change, by default, the video surface will change depending of the width and height of the movie in the file
										-- Here, I don't want the video surface to auto resize.
			reader.pos_x:=100			-- You can move the video surface by clicking on it and moving it
			reader.pos_y:=100
	...
	on_mouse_down(reader:VIDEO_READER;is_left_button, is_right_button, is_middle_button: BOOLEAN; mouse_x, mouse_y: NATURAL_16)
		do
			if is_left_button then
				if mouse_x>reader.pos_x+10 and mouse_x<reader.pos_x+reader.width-10 and
						mouse_y>reader.pos_y+10 and mouse_y<reader.pos_y+reader.height-10 then
								-- If the user drag the video, move the video reader
					move_video:=true
					mouse_pressed_x:=mouse_x
					mouse_pressed_y:=mouse_y
				end
				if mouse_x>=reader.pos_x-10 and mouse_x<=reader.pos_x+10 then
						-- If the user click on the edge of the video, resize it.
					change_video_width_left:=true
					mouse_pressed_x:=mouse_x
				end
				if mouse_x>=reader.pos_x+reader.width-10 and mouse_x<=reader.pos_x+reader.width+10 then
					-- If the user click on the edge of the video, resize it.
					change_video_width_right:=true
					mouse_pressed_x:=mouse_x
				end
				if mouse_y>=reader.pos_y-10 and mouse_y<=reader.pos_y+10 then
					-- If the user click on the edge of the video, resize it.
					change_video_height_top:=true
					mouse_pressed_y:=mouse_y
				end
				if mouse_y>=reader.pos_y+reader.height-10 and mouse_y<=reader.pos_y+reader.height+10 then
					-- If the user click on the edge of the video, resize it.
	...
	on_mouse_move(lib_ctrl:GAME_LIB_CONTROLLER;reader:VIDEO_READER;x, y: NATURAL_16)
		do
			if move_video then
					-- If the user drag the video, move the video reader
				reader.pos_x:=reader.pos_x+(x.to_integer_32-mouse_pressed_x.to_integer_32)
				reader.pos_y:=reader.pos_y+(y.to_integer_32-mouse_pressed_y.to_integer_32)
				mouse_pressed_x:=x
				mouse_pressed_y:=y

...
feature {NONE} -- Initialization
	...
	make(l_buffer_size:INTEGER)
		...
		do
			...
			read_error
			{AUDIO_EXTERNAL}.AL_gen_sources(1,$source_c)
			read_error
			...
			{AUDIO_EXTERNAL}.AL_Gen_Buffers(nb_buffer,$sound_al_buffer_c)
			read_error
			check not is_error end
...
--		end
	...
	is_playing:BOOLEAN
		...
		do
			Result:=(get_int_param_c({AUDIO_EXTERNAL}.AL_SOURCE_STATE)={AUDIO_EXTERNAL}.AL_PLAYING)
		end

	is_pause:BOOLEAN
			-- Return true if the sound source is currently on pause.
		do
			Result:=(get_int_param_c({AUDIO_EXTERNAL}.AL_SOURCE_STATE)={AUDIO_EXTERNAL}.AL_PAUSED)
		end
...
--		end
	...
	get_gain:REAL_32
		...
		do
			Result:=get_float_param_c({AUDIO_EXTERNAL}.AL_GAIN)
		end
	...
	set_gain(value:REAL_32)
		...
		do
			set_float_param_c({AUDIO_EXTERNAL}.AL_GAIN,value)
		ensure
			Source_Set_Gain_Is_Set: get_gain = value
		end

	queue_sound_loop(sound:AUDIO_SOUND;nb_loop:INTEGER)
			-- Add a `sound' to the playing queue.
			-- Put `nb_loop' to 0 for no loop and to -1 to infinite loop
		local
			sound_tuple:TUPLE[sound:AUDIO_SOUND;nb_loop:INTEGER]
		do
			create sound_tuple
	...
	queue_sound_infinite_loop(sound:AUDIO_SOUND)
			-- Add a `sound' to the playing queue.
	...
	set_direction(x,y,z:REAL)
		...
		do
			set_3_float_params({AUDIO_EXTERNAL}.AL_DIRECTION,x,y,z)
		end
	...
	get_direction:TUPLE[x,y,z:REAL]
		...
		do
			Result:=get_3_float_parms({AUDIO_EXTERNAL}.AL_DIRECTION)
		end
...
feature {NONE} -- Implementation - Routines
	...
	get_processed_buffers_number:INTEGER
		do
			Result:=get_int_param_c({AUDIO_EXTERNAL}.AL_BUFFERS_PROCESSED)
		end
		...
		do
			if channel=1 then
				if bits_resolution=8 then
					format:={AUDIO_EXTERNAL}.AL_FORMAT_MONO8
				else
					format:={AUDIO_EXTERNAL}.AL_FORMAT_MONO16
				end
			else
				if bits_resolution=8 then
					format:={AUDIO_EXTERNAL}.AL_FORMAT_STEREO8
				else
					format:={AUDIO_EXTERNAL}.AL_FORMAT_STEREO16
				end
			end

			check buffer_tail/=(buffer_head+1)\\nb_buffer end
			read_error
			{AUDIO_EXTERNAL}.AL_buffer_data(sound_al_buffer.at (buffer_head),format,l_buffer,length,freq)
			read_error
			check not is_error end
	...
	get_int_param_c(id:INTEGER):INTEGER
		local
			value:INTEGER
		do
			{AUDIO_EXTERNAL}.AL_get_source_i(index,id,$value)
			Result:=value
		end
	...
	get_float_param_c(id:INTEGER):REAL_32
		local
			value:REAL_32
		do
			{AUDIO_EXTERNAL}.AL_get_source_f(index,id,$value)
			Result:=value
		end

	set_float_param_c(id:INTEGER;value:REAL_32)
		do
			{AUDIO_EXTERNAL}.AL_set_source_f(index,id,value)
		end

	set_float_params_c(id:INTEGER;ptr:POINTER)
		do
			{AUDIO_EXTERNAL}.AL_set_source_fv(index,id,ptr)
		end

	get_float_params_c(id:INTEGER;ptr:POINTER)
		do
			{AUDIO_EXTERNAL}.AL_get_source_fv(index,id,ptr)
		end
